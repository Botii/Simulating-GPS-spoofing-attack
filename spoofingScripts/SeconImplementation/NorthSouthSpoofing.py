...
class FakeGPSModule(mp_module.MPModule):
    def __init__(self, mpstate):
        ...
        self.spoofing_active = False  # track spoofing state
            ...
            if map is not None:
                menu = MPMenuSubMenu('FakeGPS',
                                    items=[MPMenuItem('SetPos', 'SetPos', '# fakegps setpos'),
                                           ...
                                           MPMenuItem('StopSpoof', 'StopSpoof', '# fakegps stopspoof')]) # new buttons 
                map.add_menu(menu)
    ...
    def idle_task(self):
        ...
        # if the spoofing button has been clicked, start the spoofing
        if self.spoofing_active:

            burst_interval = 1.8  # burst interval 

            # check wether the burst interval has elapsed
            if now - self.last_burst_time >= burst_interval:

                # add offset to the lat
                self.FakeGPS_settings.lat += 0.000001

                # Update the map
                self.mpstate.click_location = (self.FakeGPS_settings.lat, self.FakeGPS_settings.lon)

                # Record the burst time
